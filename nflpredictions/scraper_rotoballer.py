...
articleNumber = 16
def fetch_rotoballer_data(weeknum, url, weboptions):
    ...
    try:
        ...
        print('hasattr()', rotoballer.get("searchTerm"))
        article = driver.find_element(By.TAG_NAME, "article")
        wait.until(lambda d : article.is_displayed())
        searchTerm = rotoballer.get("searchTerm")
        if searchTerm:
            picks = article.find_elements(By.XPATH, "//strong[contains(text(), '" + rotoballer['searchTerm'] + "')]/parent::*")
        else:
            picks = article.find_elements(By.XPATH, rotoballer["searchTag"])
        ...
        if len(picks) == 0:
            print('writer with no picks: ', rotoballer)
        ...
        for p in picks:
            ...
            # driver.refresh()
            # article = driver.find_element(By.TAG_NAME, "article")
            # picks = article.find_elements(By.XPATH, "//" + rotoballer['searchTag'] + "[contains(text(), '" + rotoballer['searchTerm'] + "')]/parent::*")
            # p = picks[pickNum]
            try: 
                winner = ''
                loser = ''
                ...
                try:
                    ...
                    # rotoballerrows.append(['oddsshark', awayTeam, awayTeamScore, homeTeam, homeTeamScore])
                except ValueError:
                    print(ValueError, [rotoballer['name'],winner, winnerScore, loser, loserScore])
                # pInt = pInt + 1
            except Exception as e:
                print('Exception:', e)
                # traceback.print_exc()
            pickNum = pickNum + 1
        print('rotoballerrows:', rotoballerrows)
        driver.close()
        return rotoballerrows
    except Exception as e:
        print('Exception:', e)
        traceback.print_exc()
        driver.close()
...
def main(weeknum, weboptions):
    if weeknum == None:
        weeknum = sys.argv[1]
    url = "https://rotoballer.com/nfl/nfl-stories/nfl-picks-predictions-odds-week-7-2025"
    html_content = fetch_rotoballer_data(weeknum, url, weboptions) #'https://tallysight.com/new/widget/staff-picks/usa-today-sports/nfl/event:2024-25-week-17/default:ml/types:ml,ats/extras:condensed/performances:bboverall,overall?id=5fef16ef-7f0c-41e5-81c9-a000636d9d0c'
    if html_content:
        print(html_content)
    else:
